...
public:
	...
	enum Config {
		...
		DecorationsOnly = 0x800000, Write2Color = 0x1000000, DisableDotIntersection = 0x2000000, WriteDotColor = 0x4000000, LongestPath = 0x8000000,
	};

	void go() {
		resetConfig();
		setFlag(Config::WriteDotColor);
		setFlag(Config::LongestPath);
		setGridSize(5, 5);
		setSymbol(Decoration::Start, 0, 0);
		setFlagOnce(Config::DisableWrite);
		generate(0x0A3B5, Decoration::Dot_Intersection, 36, Decoration::Gap, 12);
		set(9, 0, 0); set(10, 1, 0); set(9, 10, 0); set(10, 9, 0);
		write(0x0A3B5);
	}
